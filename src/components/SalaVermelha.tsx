...
/**
 * Cor semântica por categoria — usada APENAS no ícone e na barra lateral.
 * Fundo do card permanece branco/card para manter o visual premium sóbrio.
 */
...
export default function SalaVermelha() {
  ...
  return (
    <div className="space-y-3">
      {/* Header da Sala Vermelha */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="w-1.5 h-1.5 rounded-full bg-destructive animate-pulse" />
          <h2 className="font-heading font-semibold text-[13px] text-foreground tracking-tight">
            Sala Vermelha
          </h2>
        </div>
        <span className="text-[10px] font-heading font-medium text-muted-foreground tabular-nums">
          {filtered.length} condutas
        </span>
      </div>
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Buscar conduta crítica (ex: sepse, IAM, AVC...)"
          className="w-full pl-9 pr-4 h-10 text-[12px] font-heading rounded-xl bg-card border border-border focus:outline-none focus:ring-2 focus:ring-destructive/20 transition-all placeholder:text-muted-foreground/50"
        />
      </div>

      {/* Filtros por categoria — pills compactos e neutros */}
      <div className="flex gap-1.5 overflow-x-auto no-scrollbar -mx-1 px-1 pb-0.5">
        {(["Todos", ...categories] as const).map(cat => {
          const isActive = activeCat === cat;
          const color = cat !== "Todos" ? CAT_COLOR[cat as Category] : undefined;
          return (
            <button
              key={cat}
              onClick={() => setActiveCat(cat as Category | "Todos")}
              className={`shrink-0 px-2.5 py-1 rounded-md text-[10px] font-heading font-semibold transition-all ${
                isActive
                  ? "bg-foreground text-background"
                  : "bg-muted text-muted-foreground hover:bg-muted/80"
              }`}
              style={isActive && color ? { background: color, color: "#fff" } : undefined}
            >
              {cat}
            </button>
          );
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          ...
          <button
            ...
            className="flex items-center justify-center gap-1.5 h-9 rounded-lg bg-card border border-border text-[11px] font-heading font-semibold text-foreground hover:bg-muted/40 active:scale-[0.98] transition-all"
          >
      ...
      {/* Grid de protocolos — cards claros com ícone colorido e barra lateral */}
      {filtered.length === 0 ? (
          ...
          {filtered.map(item => {
            ...
            return (
              <button
                ...
                className="flex items-center gap-2.5 p-3 rounded-xl bg-card border border-border hover:border-border/70 hover:bg-muted/20 active:scale-[0.97] transition-all text-left"
              >
                ...
                />
                {/* Ícone com cor da categoria, fundo suave */}
                <div
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-heading font-semibold text-[12px] text-foreground leading-tight truncate">
                    {item.label}
                  </div>
                  <div className="text-[10.5px] text-muted-foreground leading-tight truncate mt-0.5">
                    {item.desc}
                  </div>
                </div>
