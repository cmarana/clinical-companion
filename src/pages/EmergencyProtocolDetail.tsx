            ...
            <TabsList className="inline-flex h-auto gap-0 bg-secondary p-1 rounded-full divide-x divide-border/40">
              {orderedSections.map(s => (
                <TabsTrigger
                  key={s.id}
                  value={s.id}
                  className="shrink-0 text-[11px] px-3 py-1.5 rounded-none first:rounded-l-full last:rounded-r-full data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-sm bg-transparent text-foreground/80 hover:text-foreground transition-colors"
                >
                  {s.title}
                </TabsTrigger>
              ))}
