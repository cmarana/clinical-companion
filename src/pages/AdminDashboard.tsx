          ...
          return (
            ...
            >
              ...
              {tab.id === "feedback" && stats.newFeedback > 0 && (
                <span className="bg-destructive text-white text-[9px] rounded-full px-1.5 py-0.5 font-bold">
                  {stats.newFeedback}
                </span>
              )}
            </button>
        ...
        <div className="px-4 py-4 space-y-4">
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              { label: "Feedbacks", value: stats.totalFeedback, color: "text-destructive", icon: MessageSquare },
              { label: "Novos", value: stats.newFeedback, color: "text-destructive", icon: Clock },
            ].map(s => {
              const SIcon = s.icon;
              return (
          ...
          {filteredUsers.map(u => (
            <div key={u.user_id} className="bg-card rounded-xl border border-border p-3">
              <div className="flex items-start gap-3">
                {u.avatar_url ? (
                  <img src={u.avatar_url} alt="" width={40} height={40} loading="lazy" decoding="async" className="w-10 h-10 rounded-full object-cover" />
                ) : (
