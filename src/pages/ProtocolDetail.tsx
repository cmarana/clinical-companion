            ...
            <TabsList className="inline-flex h-auto gap-0 bg-secondary p-1 rounded-full">
              {visibleSections.map((s) => (
                <TabsTrigger
                  key={s.id}
                  value={s.id}
                  className="shrink-0 text-xs px-3.5 py-1.5 rounded-full data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-sm bg-transparent text-foreground/80 hover:text-foreground transition-colors"
                >
                  {s.title}
                </TabsTrigger>
              ))}
