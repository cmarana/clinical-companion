  ...
  return (
    <div className="min-h-screen bg-[#0A0F1F] text-slate-100 flex flex-col items-center justify-center px-6 text-center">
      ...
      <div className="inline-flex items-center gap-2 rounded-full border border-white/25 bg-white/10 px-3 py-1 text-xs font-medium text-white">
        <Sparkles className="h-3.5 w-3.5 text-white" /> Pré-lançamento
      </div>
      <h1 className="mt-5 text-3xl sm:text-4xl font-bold tracking-tight max-w-xl text-white">
        O acesso ao app será liberado em breve.
      </h1>
      <p className="mt-4 text-base text-slate-200 max-w-md leading-relaxed">
      ...
      <div className="mt-8 flex flex-wrap gap-3 justify-center">
        <Button asChild className="bg-primary hover:bg-primary/90 text-white font-semibold rounded-full gap-2">
          <Link to="/">Voltar à página inicial <ArrowRight className="h-4 w-4" /></Link>
        </Button>
        {user && (
          <Button
            variant="outline" onClick={() => supabase.auth.signOut()}
            className="rounded-full border-white/20 bg-white/5 text-white hover:bg-white/15 gap-2"
          >
            <LogOut className="h-4 w-4" /> Sair
          </Button>
        )}
      </div>
      <p className="mt-10 text-xs text-slate-400 inline-flex items-center gap-1.5">
